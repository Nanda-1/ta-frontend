...
const InputSaksiDebitor = () => {
  ...
  const addSaksi = (e) => {
    setSaksi(true);
    if (inputApht.nik_saksi_debitor) {
      let nikNum = inputApht.nik_saksi_debitor.length;
    ...
    if (dataNik) {
      ...
      let emailDebitor = "email_saksi_debitor";
      let phoneDebitor = "tlp_saksi_debitor";
      ...
      if (
        inputApht.nik_saksi_debitor !== undefined &&
        inputApht.email_saksi_debitor !== undefined &&
        inputApht.tlp_saksi_debitor !== undefined
      ) {
        setApht({ ...inputApht });
      }
    }
  ...
  const searchKtpSaksi = () => {
    if (inputApht.nik_saksi_debitor) {
      setWarning(0);
      setLoadingFile(true);
      cekKtp(inputApht.nik_saksi_debitor, "saksi_debitor");
    }
  };
  ...
  return (
    <div className="relative w-full mb-3">
      ...
      >
        Tambahkan Identitas Saksi Pihak Pertama (Debitor)
      </label>
      ...
      {saksi && inputApht.nik_saksi_debitor === undefined ? (
      ...
      {saksi && inputApht.email_saksi_debitor === undefined ? (
        <div className="mx-auto mt--2 mb-1">
          <label className="text-xxs italic text-red-600">
      ...
      {saksi && inputApht.tlp_saksi_debitor === undefined ? (
        <div className="mx-auto mt--2 mb-1">
          <label className="text-xxs italic text-red-600">
...
export default InputSaksiDebitor;
