...
const InputSaksiPembeli = ({ next }) => {
  ...
  const addSaksi = (e) => {
    if (inputAjb.nik_pembeli && inputAjb.nik_pembeli.length !== 16) {
      ...
      if (
        inputAjb.nik_saksi_pembeli !== undefined &&
        inputAjb.email_saksi_pembeli !== undefined &&
        inputAjb.tlp_saksi_pembeli !== undefined
      ) {
  ...
  const searchKtpSaksi = () => {
    if (inputAjb.nik_saksi_pembeli) {
      setLoadingFile(true);
      cekKtp(inputAjb.nik_saksi_pembeli, "saksi_pembeli");
    }
  };
  ...
  return (
    <div className="relative w-full mb-3">
      ...
      >
        Tambahkan Identitas Saksi Pihak Kedua (Pembeli)
      </label>
      ...
      <input
        ...
        name="email_saksi_pembeli"
        onChange={handleChange}
        value={inputAjb.email_saksi_pembeli}
        autoComplete="off"
        required
      />
      ...
      <input
        ...
        name="tlp_saksi_pembeli"
        onChange={handleChange}
        value={inputAjb.tlp_saksi_pembeli}
        autoComplete="off"
        required
      />
      {warning ? (
...
export default InputSaksiPembeli;
