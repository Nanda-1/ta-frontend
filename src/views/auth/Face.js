...
// import { TRIANGULATION } from "https://codepen.io/bramus/pen/71c99fe4a6bd0c6bd7f756e1295a7e89.js";
...
export default function Face() {
  ...
  const go = async () => {
    ...
    const triggerPermissionsPrompt = async () => {
      // By trying to access a stream the Permissions Prompt will be triggered.
      // If access was already granted, no prompt will be triggered.
      try {
        let stream = await navigator.mediaDevices.getUserMedia({ video: true });
        stream(null);
      } catch (e) {
        // If the user declined access, an error will have been thrown
        // We catch this here and throw our own error
        alert("You need to allow Camera Access for this to work");
      }
    };

    const getVideoDevices = async () => {
      // Faux-call to make the permissions prompt come through …
      // Otherwise enumerateDevices() will return no devices
    ...
    // const updateVideoDevicesInUI = (videoDevices, ddm) => {
    //   ddm.length = 0; // Remove all options
    //   for (let videoDevice of videoDevices) {
    //     ddm.options[ddm.options.length] = new Option(
    //       videoDevice.label,
    ...
    const setSelectecedVideoDevice = (availableVideoDevices, deviceId) => {
      if (!availableVideoDevices.length) {
        alert("No Video Input Devices detected");
      }
    ...
    // @ref https://github.com/tensorflow/tfjs-models/blob/master/face-landmarks-detection/demo/index.js
    const distance = (a, b) => {
    ...
    const detectAndPaint = async () => {
      ...
      // ctx.drawImage(videoEL, 0, 0, videoEL.width, videoEL.height, 0, 0, ctx.canvas.width, ctx.canvas.height);
          ...
          <div className="space-y-4">
            ...
            <div className="text-center w-auto ml-12 mr-12 mx-auto">
              {capturing ? (
                ...
                <button
                  type="button"
                  className="bg-blue-500 text-white active:bg-sky-500 text-sm px-4 py-2 rounded-xl shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1 w-full ease-linear transition-all duration-150 mt-12"
